...
To do it well is a course on its own -- and we have that here, offered by the awesome Adam Marton. 

And not every story needs a publication-ready chart. Sometimes, you can help yourself and your story by just creating a quick chart, which helps you see patterns in the data that wouldn't otherwise surface. That's called exploratory data visualization, a great tool in your data analysis toolkit.
...
It's a chart designed to show differences between things -- the magnitude of one thing compared to the next thing.
...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer**
...
After we've invoked the ggplot() function, you'll notice we're using a + symbol. It means the same thing as the pipe symbol -- \|\> -- "and then do this". It makes no sense to me either, just something to live with.
...
baltco_911_calls_by_month |>
  ...
  labs(
    ...
    x = "month",
    y = "total calls",
    caption = "source: Baltimore County. Graphic by Akira Kyles. 10/27/2025"
...
Those are a little hard to read, so we can turn them 45 degrees to remove the overlap using the theme() function for styling. With "axis.text.x = element_text(angle = 90, hjust=1)" we're saying, turn the date labels 90 degrees.
