...
**Task** Run the following code in the codeblock below to load the tidyverse library and the tidycensus library (remove the comment in front of install.packages if you need to install the latter)
...
We're going to use a library called `tidycensus` which makes calls to the Census API. That means we don't have to go through the process of importing the data from a file. For someone who learned to do this from the ground up, this is pretty amazing.

First, you need to get [an API key from the Census Bureau](https://api.census.gov/data/key_signup.html). It takes a few minutes and you need to activate your key via email. Once you have your key, you need to set that for this session. Just FYI: Your key is your key. Do not share it.
...
```{r echo=FALSE}
#census_api_key("e2a96419e70d77831f3067b4691e06bc7247104e", install=TRUE)
```

The `install=TRUE` part saves your key for future use.

So to give you some idea of how complicated the data is, let's pull up just one file from the decennial Census. We'll use DHC (Demographic and Housing Characteristics File) . That has the major population and housing data.
...
```{r}
DHC <- load_variables(2020, "dhc", cache = TRUE)
View(DHC)
```


For the 2010 census, the file was SF1, or Summary File 1. That has the major population and housing data for 2010. DHC replaced that for 2020.
```{r}
...
To get a sense of what these variables mean, I turn to a few sources:
A table with all of the DHC variables in 2020: https://api.census.gov/data/2020/dec/dhc/variables.html

Census Reporter: https://censusreporter.org/ - better for general concepts
...
**Task** Run the following codeblock and examine the 2000, 2010 and 2020 results
...
```{r}
all_data <- p20 |> 
  inner_join(p10, by=c("NAME", "GEOID")) |> 
  inner_join(p00, by=c("NAME", "GEOID"))
...
all_data |> mutate(change = ((Population2020-Population2000)/Population2000)*100) |> arrange(desc(change))
```

Here are more details on the 2020 Census. The Census Bureau has a [schedule of releases](https://www.census.gov/programs-surveys/popest/about/schedule.html). There are some variables available:
...
md |>
  ...
  labs(title = "Household income by county in Maryland",
       subtitle = "2017-2023 American Community Survey",
       y = "",
