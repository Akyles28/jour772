...
**Task** Look at the uploaded data, then click on the "Visualize" button.
...
Now we make a chart. Select bar chart, which should come up by default. 
...
Let's also choose to sort the bars so that the largest value (and bar appears first). We do that by clicking on the "Sort bars" button under the "Sorting and Grouping" menu below.
...
# calculate the total number of votes and Moore's percentage, saving it back to the md_gov_county dataframe
 md_gov_county1 <- md_gov_county|>
 mutate(total_votes = rowSums(across(3:8))) |> 
 mutate(moore_pct = round((Moore/total_votes) * 100,2))


write_csv(md_gov_county1, "data/md_gov_county1.csv")
```
...
**This map is more visually appealing, easy to understand now that it shows counties instead of zip codes, because even a Maryland native like myself wouldn't be able to pinpoint each place off the top of my head from a zip code.** 
...
**https://www.datawrapper.de/_/ymxSh/** Put the URL of your map here.
