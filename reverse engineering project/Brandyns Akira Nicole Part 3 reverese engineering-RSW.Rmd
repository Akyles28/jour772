...
```{r}
armed_shootings <- fatal_police_shootings_data |> 
  filter(date > "2020-09-03", armed_with == "gun") |> 
  group_by(race_new, armed_with) |> 
  summarise(count = n()) |> 
...
```{r}
 armed_shootingsg |> 
  ...
  labs(
    ...
    x = "Weapon",
    y = "Gender",
    caption = "source: The Washington. Graphic by Akira Kyles. 10/27/2025")
```
```{r}
fatal_police_shootings_data |> 
  filter(date > "2020-09-03" &
